...
int check_collision(const GameState *g, Point next) {
    /* Wall */
    if (next.y < BOARD_TOP  || next.y >= BOARD_TOP  + BOARD_ROWS) return 1;
    if (next.x < BOARD_LEFT || next.x >  BOARD_LEFT + BOARD_COLS)  return 1;
    /* Self */
    for (int i = 0; i < g->snake.len - 1; i++) {
        if (g->snake.body[i].y == next.y && g->snake.body[i].x == next.x) return 1;
    }
    return 0;
}
...
void handle_input(GameState *g, int ch) {
    ...
    switch (ch) {
        case 'w': case 'W': case KEY_UP:    if (d != DOWN)  g->snake.dir = UP;    break;
        case 's': case 'S': case KEY_DOWN:  if (d != UP)    g->snake.dir = DOWN;  break;
        case 'a': case 'A': case KEY_LEFT:  if (d != RIGHT) g->snake.dir = LEFT;  break;
        case 'd': case 'D': case KEY_RIGHT: if (d != LEFT)  g->snake.dir = RIGHT; break;
        case 'p': case 'P': g->paused = !g->paused; break;
        case 'q': case 'Q': g->running = 0; break;
        default: break;
    }
}
...
void update(GameState *g) {
    ...
    g->snake.body[0] = next;
    if (ate) {
        if (g->snake.len < MAX_SNAKE_LEN) g->snake.len++;
        g->score += 10;
        if (g->speed > MIN_SPEED) g->speed -= SPEED_STEP;
        place_food(g);
    }
}
